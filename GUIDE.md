...
**Проверка токена при старте.** При монтировании `AuthProvider` делает `GET /apiback/auth/me` (эндпоинт защищён JWT-гардом на бэке). Если токен из cookie недействителен или просрочен — бэкенд вернёт 401, cookie чистится и `token` обнуляется. Пока идёт проверка, флаг `ready = false` — `ProtectedRoute` рендерит `null` и не перенаправляет, чтобы не мигать страницей логина у авторизованных пользователей.
...
Проверяет валидность JWT. Требует заголовок `Authorization: Bearer <token>`.

**Response:** `{ "id": "<UUID>", "login": "admin" }` — данные из payload токена.

Возвращает `401`, если токен отсутствует, подделан или просрочен. Используется `AuthContext` при старте приложения для верификации сохранённой куки.
